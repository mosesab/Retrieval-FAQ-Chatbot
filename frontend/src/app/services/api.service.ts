...
import { Options, RequestBody, PaginationParameters} from '../../types';
...
export class ApiService {
  ...
  post<T>(url: string, body: RequestBody, options: Options): Observable<T> {
    ...
    // Used to make a POST request to the API
  postGet<T>(url: string, body: PaginationParameters, options: Options): Observable<T> {
    return this.httpClient.post<T>(url, body, options) as Observable<T>;
  }
